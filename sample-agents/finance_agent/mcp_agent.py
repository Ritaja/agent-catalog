#!/usr/bin/env python3
"""
FastMCP Server for Finance Agent
This module defines the Finance Agent using the fastmcp library.

It exposes a factory function `create_mcp_server()` suitable for in-memory
testing with fastmcp.Client, and a `start_mcp_server()` runner for CLI use.
"""
import sqlite3
import pandas as pd
import argparse
...
def create_mcp_server(host: str | None = None, port: int | None = None) -> FastMCP:
    """Create and return a configured FastMCP server without running it.

    This is ideal for in-memory testing with fastmcp.Client.
    """
    # Initialize database so tools have data
    setup_db()
    ...
    server = FastMCP("Finance Agent", **kwargs)
    ...
    @server.tool
    def top_stocks_by_volume(limit: int = 3) -> str:
        """Get top stocks by trading volume"""
        return get_top_stocks_by_volume(limit)
    ...
    @server.resource("stocks://data")
    def stocks_data() -> str:
        """Complete stock dataset"""
        return get_stocks_resource()

    return server


def start_mcp_server(host: str = "0.0.0.0", port: int = 5053, transport: str = "sse"):
    """Start the FastMCP server using the specified transport."""
    print(f"🚀 Starting Finance Agent MCP server on {host}:{port} via {transport}")
    print("Tools: sql_query, stock_price, top_stocks_by_volume, market_summary")
    print("Resource: stocks://data")
    server = create_mcp_server(host=host, port=port)
    server.run(transport=transport)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Start FastMCP server for Finance Agent")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Port for the MCP server (default: 5053)"
    )
    parser.add_argument(
        '--transport', choices=['stdio', 'sse', 'streamable-http'], default='sse',
        help="Transport method for the MCP server"
    )
    args = parser.parse_args()

    start_mcp_server(host=args.host, port=args.port, transport=args.transport)
